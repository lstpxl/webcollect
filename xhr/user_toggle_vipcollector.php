...
// =============================================================================
function outhtml_script_user_toggle_vipcollector() {

$str = <<<SCRIPTSTRING

function js_user_toggle_vipcollector_query(user_id, modify) {

	var url = '/xhr/user_toggle_vipcollector.php?user_id=' + user_id + '&modify=' + modify + '';
...
// =============================================================================
function outhtml_user_toggle_vipcollector_result($param) {
	...
	$qr = mydb_queryarray("".
		" SELECT user.user_id, user.is_superadmin, user.is_vipcollector ".
		" FROM user ".
	...
	$out .= '<!--';
	$out .= 'blockid=user_toggle_vipcollector_div_id'.$param['user_id'].';';
	$out .= '-->';
	...
	if ($qr[0]['is_vipcollector'] == 'Y') {
		$str = 'vip';
		$style = ' background-color: #cc9f2c; color: #ffffff; ';
	} else {
		$str = 'не vip';
		$style = ' color: #808080; ';
	}
	...
	if ($modify) {
		$onclick_insert = ' onclick="js_user_toggle_vipcollector_query('.$param['user_id'].', \'y\'); return false; " ';
...
// =============================================================================
function outhtml_user_toggle_vipcollector_div($param) {

	$out = '';
	
	$out .= '<div id="user_toggle_vipcollector_div_id'.$param['user_id'].'">';
...
// =============================================================================
function try_update_user_toggle_vipcollector(&$param) {
	...
	$qru = mydb_query("".
		" UPDATE user ".
		" SET user.is_vipcollector = '".$newval."' ".
		" WHERE user.user_id = '".$param['user_id']."' ". 
		"");
	if (!$qru) {
...
// =============================================================================
function jqfn_user_toggle_vipcollector($param) {
...
if (!function_exists('localxhr')) {
	function localxhr($param) {
		jqfn_user_toggle_vipcollector($param);
		return true;
	}
}


if (mb_strpos('_'.$_SERVER['SCRIPT_FILENAME'], '/xhr/user_toggle_vipcollector.php') > 0) {
