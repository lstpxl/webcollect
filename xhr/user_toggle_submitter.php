...
// =============================================================================
function outhtml_script_user_toggle_submitter() {

$str = <<<SCRIPTSTRING

function js_user_toggle_submitter_query(user_id, modify) {

	var url = '/xhr/user_toggle_submitter.php?user_id=' + user_id + '&modify=' + modify + '';
...
// =============================================================================
function outhtml_user_toggle_submitter_result($param) {
	...
	$qr = mydb_queryarray("".
		" SELECT user.user_id, user.is_superadmin, user.can_submit_item ".
		" FROM user ".
	...
	$out .= '<!--';
	$out .= 'blockid=user_toggle_submitter_div_id'.$param['user_id'].';';
	$out .= '-->';
	...
	if ($qr[0]['can_submit_item'] == 'Y') {
		$str = 'может загружать';
		$style = ' background-color: #14a314; color: #ffffff; ';
	} else {
		$str = 'не загружает';
		$style = ' color: #808080; ';
	}
	...
	if ($modify) {
		$onclick_insert = ' onclick="js_user_toggle_submitter_query('.$param['user_id'].', \'y\'); return false; " ';
...
// =============================================================================
function outhtml_user_toggle_submitter_div($param) {

	$out = '';
	
	$out .= '<div id="user_toggle_submitter_div_id'.$param['user_id'].'">';
...
// =============================================================================
function try_update_user_toggle_submitter(&$param) {
	...
	$qru = mydb_query("".
		" UPDATE user ".
		" SET user.can_submit_item = '".$newval."' ".
		" WHERE user.user_id = '".$param['user_id']."' ". 
		"");
	if (!$qru) {
...
// =============================================================================
function jqfn_user_toggle_submitter($param) {
...
if (!function_exists('localxhr')) {
	function localxhr($param) {
		jqfn_user_toggle_submitter($param);
		return true;
	}
}


if (mb_strpos('_'.$_SERVER['SCRIPT_FILENAME'], '/xhr/user_toggle_submitter.php') > 0) {
