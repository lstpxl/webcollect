...
function smsc_init() {
	$filename = '/home/wkh/smsc_conf_webcollect';
	$fhandle = fopen($filename, "rb")
		or die('Could not read ENV file (smsc_conf_webcollect)');
	$contents = fread($fhandle, filesize($filename))
		or die('Could not read ENV file (smsc_conf_webcollect)');
	fclose($fhandle);
	parse_str($contents, $arr);
	
	if (!isset($arr['SMSC_LOGIN'])) die('Error in smsc_conf_webcollect');
	if (!isset($arr['SMSC_PASSWORD'])) die('Error in smsc_conf_webcollect');

	define("SMSC_LOGIN", $arr['SMSC_LOGIN']);
